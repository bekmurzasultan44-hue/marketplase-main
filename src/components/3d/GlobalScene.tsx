...
function TestimonialsModel() {
  const groupRef = useRef<THREE.Group>(null);
  const spheresRef = useRef<THREE.Group>(null) as MutableRefObject<THREE.Group>;
  ...
  useFrame((state) => {
    ...
    spheres.children.forEach((child, i) => {
      if (i === 0) return;
      const angle = (i / (spheres.children.length - 1)) * Math.PI * 2;
